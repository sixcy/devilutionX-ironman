...
void CheckQuests()
{
	...
	if (IsIronman) {
		...
		if (IMQuestActive(ltbanner) && MyPlayer->TryRemoveInvItemById(IDI_BANNER)) {
		...
		if (IMQuestActive(rock) && MyPlayer->TryRemoveInvItemById(IDI_ROCK)) {
		...
		if (mushroom._qvar1 != QS_MUSHGIVEN && MyPlayer->TryRemoveInvItemById(IDI_MUSHROOM)) {
			SpawnQuestItem(IDI_SPECELIX, MyPlayer->GetTargetPosition(), 0, 0);
			InitQTextMsg(TEXT_MUSH12);
			mushroom._qvar1 = QS_MUSHGIVEN;
		}
		if (mushroom._qactive != QUEST_DONE && mushroom._qvar1 == QS_MUSHGIVEN && MyPlayer->HasItem(IDI_SPECELIX)) {
			AllItemsList[IDI_SPECELIX].iUsable = true;
			mushroom._qactive = QUEST_DONE;
		}

		auto &anvil = Quests[Q_ANVIL];
		if (IMQuestActive(anvil) && MyPlayer->TryRemoveInvItemById(IDI_ANVIL)) {
		...
		if (betrayer._qactive == QUEST_INIT && MyPlayer->TryRemoveInvItemById(IDI_LAZSTAFF)) {
		...
		if (IMQuestActive(veil) && MyPlayer->TryRemoveInvItemById(IDI_GLDNELIX)) {
	...
	for (auto &quest : Quests) {
		if (quest._qidx != Q_BETRAYER && currlevel == quest._qlevel && quest._qslvl != 0) {
			...
			if (EntranceBoundaryContains(quest.position, cursPosition)) {
				InfoString = IsIronman && !CurrentLevelIsClear() ? SelectImMsg() : fmt::format(_(/* TRANSLATORS: Used for Quest Portals. {:s} is a Map Name */ "To {:s}"), _(QuestTriggerNames[ql]));
